...
export class TempSaveSampleComponent extends BaseComponent implements OnInit {
  ...
  tempSaveList = [] as SampleSaveDataArg[]; // 站存加入
  ...
  save(ref: any) {
    ...
    //先塞入站存
    this.tempSaveList.push(this.sample);

    this.message.showSucessMSG(`目前暫存比數: ${this.tempSaveList.length}`);
    console.log('目前暫存狀況', this.tempSaveList);
  ...
  mapcTypeOptions(key: any) {
    const mapObj = this.cTypeOptions.filter((x) => x.value === key);
    if (mapObj.length > 0) return mapObj[0].text;
    else return '';
  }

  mapcType2Options(key: any) {
